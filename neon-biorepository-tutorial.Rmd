...
# Create a data frame of occurrence records for hair samples in our dataset so far

hairMatches <- matches %>% 
                left_join(hair,join_by("hair"=="catalogNumber")) 

# Summarise to site by species combination and keep only those combinations for which we can obtain more than 10 individuals with paired samples
...
# Filter the full data frame to contain only those samples that match ethe above criteria
...
Now that we have filtered to the site by species combinations for which we _can_ obtain 10 individuals, we need to subset these samples to include _only_ include 10 samples for each of these combinations. 
