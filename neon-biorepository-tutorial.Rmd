...
```{r clean environment, message=FALSE, warning=FALSE, eval=TRUE}
# clean out workspace
...
Because we are interested in  the Northeastern United States, we will subset the available data to sites in NEON Domain 01: BART and HARV.
...
```{r download Data New},echo= FALSE,message=FALSE,eval=TRUE}

 NEON.cfc <- loadByProduct(dpID="DP1.10026.001",
                    include.provisional=TRUE,
                    site=c('BLAN','BART','HARV','SCBI','SERC'),
                    tabl="cfc_carbonNitrogen",
                    check.size=FALSE)
 
 NEON.ltr <- loadByProduct(dpID="DP1.10033.001",
                    include.provisional=TRUE, site=c('BLAN','BART','HARV','SCBI','SERC'),
...
```{r join cfc and ltr data},message=FALSE,eval=TRUE}
...
```{r filter cfc and ltr data},message=FALSE,eval=TRUE}
...
Up to date results for the same search terms can be found at any time <a href="https://biorepo.neonscience.org/portal/collections/list.php?db=26%2C27&datasetid=2%2C13&includeothercatnum=1&usethes=1&taxontype=1" target="_blank">here</a>. 
...
```{r investigate a biorepo download},message=FALSE,eval=TRUE}
...
```{r filter for uncertain identification},message=FALSE,eval=TRUE}
...
```{r match hair and fecal samples},message=FALSE,eval=TRUE}
...
We find nearly 150 unique individuals with paired hair and fecal samples meeting our criteria so far. Let's summarize by the number of samples across site by species combinations and filter to those combinations for which 10 or more individuals are available.
...
```{r filter based on available sample size},message=FALSE,eval=TRUE}
...
```{r choose the sample},message=FALSE,eval=TRUE}
...
We now have a list of 60 samples we could request from the NEON Biorepository via the <a href="https://biorepo.neonscience.org/portal/misc/samplerequest.php" target="_blank">Sample Request Form</a>.
...
speciesByCN <- basemap %>% 
				  ...
				  type ='pie',
				  chartdata = mapSummaryWithSpecies[,10:11],
				  width = mapSummaryWithSpecies$meanCN.cfc/3)
